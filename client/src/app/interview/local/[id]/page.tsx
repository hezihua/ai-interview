...
import {
  getLocalInterviewPrep,
  type LocalInterviewPrep,
} from "@/lib/interview-prep-local";

export default function LocalInterviewPrepPage() {
  const params = useParams<{ id: string }>();
  const id = decodeURIComponent(params.id || "");
  const [doc, setDoc] = useState<LocalInterviewPrep | null>(null);
  const [ready, setReady] = useState(false);

  useEffect(() => {
    setDoc(id ? getLocalInterviewPrep(id) : null);
    setReady(true);
  }, [id]);
      ...
      <div className="min-h-dvh bg-[linear-gradient(180deg,#071018_0%,#0b1220_100%)] px-4 py-10 text-zinc-100">
        <div className="mx-auto max-w-3xl rounded-2xl border border-white/8 bg-[#121c2b] px-5 py-8">
          ...
          <p className="mt-2 text-sm text-zinc-500">
            本机 localStorage 里没有这份面试准备，可能已被清除。
          </p>
          <Link
            href="/interview"
